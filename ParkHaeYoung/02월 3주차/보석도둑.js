...
class MaxHeap {
    ...
    push(value) {
        // 맨뒤에 추가 max heap 이므로 부모랑 비교해서 큰값을 부모랑 swap
    ...
    // 큐이기 때문에 삭제는 항상 루트노드부터. 루트 노드를 삭제하고, 맨마지막 인덱스를 루트랑 교환
    pop() {
...
// 보석 개수, 가방 개수, n개의 보석 무게와 가격 목록, k개의 가방에 담을 수 있는 보석 무게
function solution(n, k, jewel, bag) {
    const maxHeap = new MaxHeap();
    let result = 0;

    let j = 0;
    for(let i = 0; i < k; i++) {
        // 보석의 무게와 '현재 가방'에 담을 수 있는 무게를 비교
...
// [무게, 가격]
const jewel = input.splice(0, N).sort((a, b) => a[0] - b[0]);
// 담을 수 있는 무게
const bag = input.map(v => v[0]).sort((a, b) => a - b);

solution(N, K, jewel, bag);
